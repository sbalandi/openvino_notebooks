...
class Data:
    ...
    def collect_data(self, text, key_source=URL_PATH_KEY, min_max_key=VIEWS_UNIQUE_KEY):
        ...
        target_months_date = datetime.now() - relativedelta(months=+MONTH_TO_VIEW)
        satistic_date = datetime.strptime(date, "%Y-%m-%d")
        if satistic_date < target_months_date:
            return

        self._date_info.setdefault(
            date,
            {
                "all_total": 0,
                "all_unique": 0,
        ...
        for row in reader:
            ...
            if key_source == URL_PATH_KEY:
                ...
                if (
                    Path(source_path).parent.name != "notebooks"
                    and ".ipynb" != Path(source_path).suffix
                ):
                    continue
                parent_name = Path(source_path).parent.name
                if ".ipynb" == Path(source_path).suffix and self._sources_info.get(
                    parent_name
                ):
                    for key, item in self._sources_info[parent_name].items():
                        ...
                        self._date_info[key]["all_unique"] -= int(
                            item[VIEWS_UNIQUE_KEY]
                        )
                    ...
                    for _, info in self._sources_info.items():
                        ...
                        if (
                            ".ipynb" == Path(info["full_path"]).suffix
                            and Path(info["full_path"]).parts[0:3]
                            == Path(source_path).parts[0:3]
                            and Path(info["full_path"]).parts[4:7]
                            == Path(source_path).parts[4:7]
                        ):
                            the_same_dir = True
                            break
                    if the_same_dir:
            ...
            self._sources_info.setdefault(
                source_path, {"all_total": 0, "all_unique": 0, "full_path": ""}
            )
            self._sources_info[source_path][date] = {
                VIEWS_TOTAL_KEY: int(views_total),
                VIEWS_UNIQUE_KEY: views_unique,
            }
...
class ExelBuiler:
    ...
    def create_paths_snapshoot_sheet(self, sheet_name: str, data: Data):
        ...
        for i, title in enumerate(
            ["Source.Name", "Url.Path", "Total view", "Unique view"]
        ):
            worksheet.cell(1, i + 1, title)
            worksheet.cell(1, i + 1).fill = openpyxl.styles.PatternFill(
        ...
        for data_info in sorted_date_info:
            ...
            for path_name, path_info in sources_info.items():
                ...
                worksheet.append(
                    (
                        data_info[1]["source_name"],
                        path_name,
                        path_info[data_info[0]][VIEWS_TOTAL_KEY],
                        path_info[data_info[0]][VIEWS_UNIQUE_KEY],
                    )
                )
                ...
                if first_row:
                    ...
                    for cell in range(worksheet.max_column):
                        worksheet.cell(worksheet.max_row, cell + 1).fill = (
    ...
    def create_chart_sheet(self, data_list):
        ...
        for data in data_list:
            ...
            dates = openpyxl.chart.Reference(
                worksheet,
                min_col=1,
                min_row=row_offset + 2,
                max_row=worksheet.max_row,
                max_col=(len(sorted_date_info) + 1),
            )
            c2.add_data(dates, from_rows=True, titles_from_data=True)
            dates2 = openpyxl.chart.Reference(
                worksheet,
                min_col=2,
                min_row=row_offset + 1,
                max_row=row_offset + 1,
                max_col=(len(sorted_date_info) + 1),
            )
            ...
            for i in range(0, len(c2.series)):
                ...
                s1.graphicalProperties.solidFill = (
...
def get_contents_from_gh(token):
    ...
    contents = repo.get_contents(
        f"{STATISTRIC_FOLDER_ROOT}/ghrs-data/snapshots", ref=BRANCH
    )
    return contents
...
def push_table_to_gh(file_name, token):
    ...
    logger.info(
        f"xlsx path: {STATISTRIC_FOLDER_ROOT}/statistics/statistics_{current_dateTime}.xlsx"
    )
...
if __name__ == "__main__":
    ...
    for content in contents:
        ...
        if "top_paths_snapshot" in content.name:
            paths_data.collect_data(
                text, key_source=URL_PATH_KEY, min_max_key=VIEWS_UNIQUE_KEY
            )
        elif "top_referrers_snapshot" in content.name:
            referrs_data.collect_data(
                text, key_source=REFERRER_KEY, min_max_key=VIEWS_UNIQUE_KEY
            )
